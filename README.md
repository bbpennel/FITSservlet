...
<br>3. (optional) Rather than using the default log4j2.xml file located within the WAR file here: `/src/main/resource/` (which logs to a file within the Tomcat directory structure) it's possible to set up logging to point to an external log4j2 config file. Add a "log4j2.configurationFile" property to `catalina.properties` pointing to this file. It can be either a full path or have the `file:` protocol at the beginning of the entry. This is managed by the class `edu.harvard.hul.ois.fits.service.listeners.LoggingConfigurator.java`.
...
- `log4j2.configurationFile=/path/to/log4j2.xml` or `log4j2.configurationFile=file:/path/to/log4j2.xml` (optional -- to override using the default log4j2.xml in the WEB-INF/classes directory of the WAR file.) 
